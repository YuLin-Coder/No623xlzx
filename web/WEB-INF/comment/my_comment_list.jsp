		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<c:forEach items="${commentList}" var="vo" >
				...
				<td class="f-14 td-manage">
					...
					<c:if test="${type==2}">
				<c:if test="${vo.flag==1}">
						<a style="text-decoration:none" class="ml-5" href="ReplyServlet?action=toAddTeacherReply&sid=${vo.student.id}" onclick="Hui_admin_tab(this)"   title="回复" >回复</a>
					</c:if>
						<%--<c:if test="${flag==2}">
...
function del_row(obj,id){
    ...
    if(confirm("您确定要删除该条记录吗？")){
        //访问路径
        location.href="${pageContext.request.contextPath}/CommentServlet?action=delete2&id="+id;
    }
}
